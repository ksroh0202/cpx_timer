...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _saveRecords() async {
    final prefs = await SharedPreferences.getInstance();
    final rawList = _records.map((e) => jsonEncode(e.toMap())).toList();
  ...
  String get _mainTimeText {
    if (_phase == TimerPhase.prep || _phase == TimerPhase.pausedPrep) {
      return _formatSeconds(_prepRemaining);
    }
    if (_phase == TimerPhase.exam ||
        _phase == TimerPhase.pausedExam ||
        _phase == TimerPhase.finished) {
      return _formatSeconds(_examRemaining);
    }
    return _formatSeconds(examTotalSeconds);
  }
        ...
        child: Column(
          ...
          children: [
            ...
            _StageRow(
              ...
              value: _formatSeconds(_previewStageSeconds(ExamStage.historyTaking)),
            ),
            const Divider(height: 20),
            _StageRow(
              label: ExamStage.physicalExam.label,
              value: _formatSeconds(_previewStageSeconds(ExamStage.physicalExam)),
            ),
            const Divider(height: 20),
            _StageRow(
              label: ExamStage.patientEducation.label,
              value: _formatSeconds(_previewStageSeconds(ExamStage.patientEducation)),
            ),
            const Divider(height: 20),
            _StageRow(
              label: '총 사용 시간',
              value: _formatSeconds(_examElapsed),
              isBold: true,
            ),
          ],
        ),
        ...
        return Card(
          ...
          child: ListTile(
            ...
            subtitle: Padding(
              ...
              child: Text('${record.endType}  ·  ${_formatSeconds(record.totalSeconds)}'),
            ),
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    _formatSeconds(record.totalSeconds),
                    style: const TextStyle(
                      fontSize: 60,
                      fontWeight: FontWeight.w800,
                      height: 1,
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  _StageRow(
                    label: '병력 청취',
                    value: _formatSeconds(record.historySeconds),
                  ),
                  const Divider(height: 20),
                  _StageRow(
                    label: '신체 진찰',
                    value: _formatSeconds(record.physicalSeconds),
                  ),
                  const Divider(height: 20),
                  _StageRow(
                    label: '환자 교육',
                    value: _formatSeconds(record.educationSeconds),
                  ),
                  const Divider(height: 20),
                  _StageRow(
                    label: '총 사용 시간',
                    value: _formatSeconds(record.totalSeconds),
                    isBold: true,
                  ),
                ],
              ),
